...
import pdb
# RL
from agent.RL_env import h0_compute_reward,h1_compute_reward, formulate_state,compute_reward
...
def main():
    ...
    algs = ['Origin', 'Basic',  'Residual', 'Dense',]
    # algs = ['DQN', 'Residual-4',]
    ...
    for alg in tqdm(algs):
        ...
        for _ in range(avg_times):
            ...
            if alg in ['Origin', ]:
                ...
                fixed_mode = ['Basic', 'Residual', 'Dense'].index(alg)
                max_fixed_depth = 8
            ...
            for ep in tqdm(range(epochs)):
                ...
                if alg == 'Origin' and ep / epochs > 0.3:
                ...
                generated_scores.append(np.mean(metrics['val.generated_score']))
                ...
                h0_reward=h0_compute_reward(psnr_s[-1], ssim_s[-1], consumptions[-1])
                h1_reward=h1_compute_reward(psnr_s[-1], ssim_s[-1],)
                # 更新状态
        ...
        #                         ['Encoder mse loss ', 'Bits per pixel', 'Reward', 'PSNR', 'SSIM', 'Consumption', ]):
            ...
            generated_scores_avg.append(generated_scores)
        ...
        if alg == 'Origin':
            ...
            for data, y_label in zip([h0_reward_avg, h1_reward_avg,psnr_avg, ssim_avg, consumption_avg,bpp_avg,cover_scores_avg,generated_scores_avg,message_density_avg, uiqi_avg, rs_avg, mse_avg],
                                ['H0_Reward','H1_Reward', 'PSNR', 'SSIM', 'Consumption', 'Bpp','Cover_Score','Generated_Score','Message_Density','UIQI', 'RS test', 'MSE loss']):
                plt.figure()
                plt.plot(data)
